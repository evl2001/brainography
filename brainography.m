...
%     end
    ...
    elseif size(filename,1) > 1
        ...
        V=spm_vol([pathname filesep filename]);
        % if multiple volumes per img, prompt user to choose
        if length(V) > 1
            imgChoice=[];
            while isempty(imgChoice) || imgChoice > length(V) || imgChoice < 1
        ...
        if ~isequal(handles(end).dim, V(1).dim) || ~isequal(handles(end).mat, V(1).mat)
            %         end
